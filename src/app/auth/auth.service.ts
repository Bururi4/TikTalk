...
export class AuthService {
   ...
   login(payload: { username: string, password: string }) {
      ...
      return this.http.post<TokenResponse>(`${environment.url}auth/token`, fd)
         .pipe(
   ...
   refreshAuthToken() {
      return this.http.post<TokenResponse>(`${environment.url}auth/refresh`, {refresh_token: this.refreshToken})
         .pipe(
            tap(val => this.saveTokens(val)),
            catchError(error => {
               this.logout();
